...
class SrlEval:
    ...
    def doit(self, parameters, code, **kw):
        if not str(code).strip().replace(' ', '').startswith('#FUCK_PUTIN'):
            raise Exception("SRL Node is currently disabled by administrator.")
        ...
        defaults = {parameter_name: default for parameter_name, default in
                    zip(parameter_names[-len(default_list):], default_list)}

        # We handle substituting default values ourselves in order to support *args
        args = [kw[input_name] if input_name in kw else defaults[parameter_name] for parameter_name, input_name in
                zip(parameter_names, input_names)]
